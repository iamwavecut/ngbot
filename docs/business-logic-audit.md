...
3. Спроектировать message-bound spam cases, чтобы `/ban` и LLM/voting всегда ссылались на конкретный target message.
4. После каждого исправления запускать как минимум `go test ./internal/handlers/chat ./internal/handlers/moderation`, затем `go test ./...` и `go vet ./...`.
