---
last_mapped: 2026-04-15T22:00:00Z
total_files: 81
total_tokens: ~125000
---
...
> Auto-generated by Cartographer. Last mapped: 2026-04-15 (update 2)
...
│   │       ├── client_known_non_members.go  # Known-non-member CRUD + lookup (NEW)
│   │       ├── client_known_non_members_test.go  # Known-non-member CRUD lifecycle tests (NEW)
...
| `update_processor.go` | Update dispatch to handlers (chain of responsibility) | `UpdateProcessor`, `RegisterUpdateHandler()`, 19 `MessageType` constants, `StageOverrideCheck`, `StageKnownNonMember` (NEW) |
...
[3] IsChatNotSpammer? → YES → Insert member, SKIP (override)
    ...
    NO
    ▼
[4] IsChatKnownNonMember? → YES → handleKnownNonMember (re-verify, may SKIP) (NEW)
    │
    NO
    ▼
...
[7] Extract content → EMPTY → SKIP
    ...
    HAS CONTENT
    ▼
[8] detectFirstMessageExternalQuoteHeuristic → TRIGGERED → immediate spam
    │
    NOT TRIGGERED
    ▼
...
[10] Load per-chat spam examples (max 20)
    │
    ▼
[11] LLM IsSpam (few-shot: system + ~30 examples + extra + candidate)
    │
    ├── SPAM → processSpam (with voting) or processBanned (without)
    │         + rememberAuthorIfPossible (remember non-member or promote) (NEW)
    └── NOT SPAM → insertMemberIfPresent
                    + rememberAuthorIfPossible (NEW)
```
...
| `spam_detector_test.go` | LLM prompt construction tests |
| `spam_control_known_non_member_test.go` | Known-non-member clearing on spam verdict tests (NEW) |
...
| `sqlite/client_known_non_members.go` | Known-non-member CRUD + lookup (NEW) |
| `sqlite/client_known_non_members_test.go` | Known-non-member CRUD lifecycle tests (NEW) |
...
sequenceDiagram
    ...
    alt Not a member
        ...
        alt Override active
            ...
            Reactor->>DB: IsChatKnownNonMember? (NEW)
...
5. **Interface**: `dependencies.go` (3 new methods on `Client`)
...
- **Features**:
  ...
  - Reactor produces records via `rememberAuthorIfPossible`: after spam check, re-checks membership via `GetChatMember`
  - Reactor reads via `handleKnownNonMember`: remembered users skip full spam pipeline, only re-verify membership
  - SpamControl clears records via `clearKnownNonMember` on confirmed spam (`ProcessBannedMessage`, `ResolveCase`)
  - Three lifecycle outcomes: promoted to member (deleted), kicked (deleted), confirmed spammer (deleted)
- **DB**: New `chat_known_non_members` table with composite PK `(chat_id, user_id)`
- **Pipeline**: New `StageKnownNonMember` stage early in the message pipeline (after override check, before ban check)
- **Gotcha**: `UpsertChatKnownNonMember` mutates input struct in-place (`CreatedAt`/`UpdatedAt` fields)
