	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				_, _ = a.bot.RequestWithContext(ctx, api.NewChatAction(chatID, api.ChatTyping))
			}
		}
	}()
	_, _ = a.bot.RequestWithContext(ctx, api.NewChatAction(chatID, api.ChatTyping))
